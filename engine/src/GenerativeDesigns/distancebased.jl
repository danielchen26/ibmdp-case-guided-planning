...
# end
function Variance(; initial = nothing)
    function (data; prior)
...
function DistanceBased(
    ...
    conditional_range = Dict(),  # ! NEW: Add conditional_range parameter
)
    prior = Weights(prior)
    ...
    # Define compute_conditional_weights function before sampler
    # global has_logged_info = false
    compute_conditional_weights =
        function (evidence::Evidence, conditional_range = conditional_range)
            computed_weights = compute_weights(evidence)
            valid_weights = zeros(length(computed_weights))
            if !isempty(conditional_range)
                # global has_logged_info
                # if !has_logged_info
                #     @info "conditional weights function is active"
                #     has_logged_info = true
                # end
                valid_indices = trues(nrow(data))

                for (colname, range) in conditional_range
                    if colname in names(data)
                        valid_indices .&=
                            (data[!, colname] .>= range[1]) .&
                            (data[!, colname] .<= range[2])  # Fixed: inclusive upper bound
                    else
                        error("Column $colname not found in data.")
                    end
                end

                valid_weights[valid_indices] = computed_weights[valid_indices]
                if sum(valid_weights) == 0
                    error(
                        "No valid weights after applying constraints. Adjust conditional_range.",
                    )
                end
                ...
                # global has_logged_info
                # if !has_logged_info
                #     @info "unconditional weights function is active"
                #     has_logged_info = true
                # end
                valid_weights .= computed_weights
            end

            return Weights(valid_weights / sum(valid_weights))
        end

    # ! ENHANCED: Sampler function with conditional support and target value return
    sampler = function (
        evidence::Evidence,
        columns,
        rng = default_rng(),
        ...
        # Use compute_conditional_weights to support conditional sampling
        normalized_weights = compute_conditional_weights(evidence, conditional_range)

        # Sample an index according to the normalized valid weights
        index = sample(rng, normalized_weights)

        observed = data[index, :]
        target_col = target isa AbstractVector ? target[1] : target
        
        # ! NEW: Return observation, target value, and weights (consistent with conditional CEED expectations)
        return Dict(c => observed[c] for c in columns), observed[Symbol(target_col)], normalized_weights
    end


    f_uncertainty = uncertainty(data[!, target]; prior)
    compute_uncertainty = function (evidence::Evidence)
        return f_uncertainty(compute_conditional_weights(evidence)) # compute_conditional_weights becomes compute_weights function when no conditional_range is available
    end

    return (;
        sampler,
        uncertainty = compute_uncertainty,
        weights = compute_conditional_weights,
    )
end
